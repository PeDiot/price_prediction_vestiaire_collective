...
def make_dataset(num_pages: int = 6): 
    ...
    for no in range(num_pages+1):
        for brand in get_brands(): 
            basic_items_df = pd.read_json(f"./backup/brands/{brand}/items/basic_items_p{no}.json")
            items_attrs_df = pd.read_json(f"./backup/brands/{brand}/items/items_attrs_p{no}.json")
